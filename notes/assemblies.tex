...
\begin{definition}
  An \defemph{assembly} over an untyped pca~$\AA$ is a pair $S = (\S{S}, {\rz[S]})$ where $\S{S}$ is a set and $\rz[S]$ is a relation between~$\AA$ and~$\S{S}$, such that for every $x \in \S{S}$ there is $r \in \AA$ and $r \rz[S] x$.
\end{definition}
...
\begin{definition}
  A \defemph{modest} assembly $S$, also called a \defemph{modest set},\sidenote{The terminology was suggested by Dana Scott. It refers to the fact that the cardinality of a modest set~$S$ does not exceed the cardinality of $\xAtyp{S}$.} is an assembly in which elements do not share realizers:
  %
  \begin{equation*}
    \all{r \in \xAtyp{S}}{
      \all{x,y \in \S{S}}
