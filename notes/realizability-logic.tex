...
\begin{proposition}
  ...
  A predicate $p \in \Pred{S}$ is decidable if, and only if, it is classical and there exists
...
\begin{proof}
  If $\R{r} \rz p \lor \lnot p$ then it is classical by intuitionistic reasoning, and we may take
  %
  \begin{equation*}
    \R{d} = \tpcalam{\R{x}}{\T{S}} \combCase\,(\R{r}\,\R{x})\,\combTrue\,\combFalse.
  \end{equation*}
  %
  Conversely, suppose $p$ is classical and $\R{d}$ with the stated property exists. By \cref{prop:when-stable} there exists $\R{q} \in \subAtyp{\T{S} \to \T{p}}$ such that, if $\R{x} \rz[S] x$ and $p x \neq \emptyset$ then $\R{r}\,\R{x} \rz p x$. Now the realizer
  %
  \begin{equation*}
    \tpcalam{\_}{\unit}
    \tpcalam{\R{x}}{\T{S}}
